...
def create_message_collector(external_buffer: Optional[Deque[str]] = None):
    """
...
async def _load_script_with_global_buffer(session: frida.core.Session, initial_script: Optional[str], init_delay_seconds: float = 0.0) -> bool:
...
async def attach(
    ...
    try:
        ...
        if initial_script:
            try:
                await _load_script_with_global_buffer(session, initial_script)
            except Exception as e:
        ...
        result = {
            ...
            "script_loaded": initial_script is not None
        }
...
async def spawn(
    ...
    Args:
      - package_name: 应用包名
      - initial_script: 可选注入的 Frida JS（恢复前注入）
    ...
    try:
        ...
        if initial_script:
            try:
                await _load_script_with_global_buffer(session, initial_script, init_delay_seconds=0.1)
            except Exception as e:
                _frida_log(f"script load error: {e}")
                return {"status": "error", "message": str(e)}
